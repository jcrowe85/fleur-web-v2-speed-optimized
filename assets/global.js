...
(function () {
  function enableShippingProtection() {
    const addon = [...document.querySelectorAll('.upcart-public-addons')]
      .find((el) => el.textContent.includes('Shipping Protection'));

    if (!addon) return false;

    const input = addon.querySelector('input[type="checkbox"][role="switch"]');
    const label = addon.querySelector('label.upcart-public-addons__toggle-switch');
    const slider = addon.querySelector('.upcart-public-addons__toggle-slider');
    ...
    const target = slider || label || input;
  ...
  function watchForProtection(maxMs = 8000) {
    const startedAt = Date.now();

    const interval = setInterval(function () {
      const done = enableShippingProtection();
  ...
  document.addEventListener('click', function (event) {
    const addToCartClick = event.target.closest(
      'button[name="add"], .product-form__submit, [data-add-to-cart], .buy-once-link, .variant-card'
    );

    if (addToCartClick) {
      setTimeout(function () {
        watchForProtection(10000);
      }, 600);
    }
  });

  document.addEventListener('DOMContentLoaded', function () {
    watchForProtection(5000);
  });

  const observer = new MutationObserver(function () {
    if (document.querySelector('.upcart-public-addons')) {
      watchForProtection(5000);
    }
  });
