...
(function () {
  let hasAutoToggledProtection = false;

  function autoToggleShippingProtection() {
    if (hasAutoToggledProtection) return;

    const addon = Array.from(document.querySelectorAll('.upcart-public-addons'))
      .find((el) => el.textContent.includes('Shipping Protection'));

    if (!addon) return;

    const input = addon.querySelector('.upcart-public-addons__toggle-switch input[type="checkbox"]');
    const label = addon.querySelector('.upcart-public-addons__toggle-switch');

    if (!input || !label) return;

    if (input.getAttribute('aria-checked') === 'true' || input.checked) {
      hasAutoToggledProtection = true;
      return;
    }
  ...
  function runProtectionWatcher() {
    setTimeout(autoToggleShippingProtection, 500);
    setTimeout(autoToggleShippingProtection, 1000);
    setTimeout(autoToggleShippingProtection, 2000);
    setTimeout(autoToggleShippingProtection, 3500);
  }

  document.addEventListener('DOMContentLoaded', runProtectionWatcher);

  document.addEventListener('click', function () {
    runProtectionWatcher();
  });
